...
- **Janela de apuração para NF (2026-07-01):** o fechamento de cada mês NÃO é mais o mês-calendário cheio. Helper `_finPeriodo(anoMes)` → `{ini,fim}` ISO inclusivos: **≤ jun/2026** = mês cheio (modelo antigo, passado intocado); **jul/2026** = transição `01/07–15/07` (jun já fechou cheio); **≥ ago/2026** = **dia 16 do mês anterior a dia 15 do corrente** (Ago=16/07→15/08, etc; vira o ano corretamente). `_finPeriodoLabel(anoMes)` → "16/07 a 15/08". `calcFinanceiroMes` filtra rodadas (sims por data) e simulados extras (alunos por `dataAgendada`) por essa janela; `_mentSobreposicaoMes` usa a janela como recorte E como base do rateio (denominador = dias da janela). **Salário fixo e mentoria continuam integrais** por mês (não caem pela metade em julho — decisão do Tiarlles). UI: badge `📅 Apuração 16/07 a 15/08` no status; emails de NF/RPA incluem o período no `mes`. Não-alinhado ainda: pagamento de caso clínico (`_lancarPagamentoCaso`) banca por `_curMesISO` (mês-calendário), não pela janela.
...
**3. Geração de Cronograma (`cronoGer`).** Coleção `cronoTurmas/{id}`: `{nome, dataInicio, dataFim, simulados:[{id,nome,ini,fim}], provas:[{id,nome,data}], revisaoPreProva:[{id,nome,data}], semanas:[{dias,temas[],sobra[],revisoes[],aviso,nota}]}`. Semanas geradas de dataInicio+dias acumulados; datas/eventos calculados (`_cronoTurmaSemanas`, que também propaga `aviso`). **Simulado** = intervalo (badge na semana); **prova** e **revisão pré-prova** = 1 dia por entrada, ficam FORA das semanas. Cada semana tem `temas` (prioritários), `sobra` ("se sobrar tempo"), `revisoes` e **`aviso`** (mensagem livre por semana — botão "Adicionar aviso" ao lado de "Adicionar revisão", modal `#cg-aviso-modal`, exibida **em negrito** num box abaixo dos temas, na tela e no PDF; se não houver temas, aparece sozinha). Picker de temas em árvore com toggle prioridade/ordem-do-Ponto, seleção MÚLTIPLA; tema não repete entre `temas`/`sobra` (revisão é livre, mostra "(visto na semana X)"). Temas exibidos **1 por linha, em ordem numérica** (`_cronoSortTemaIds`) tanto na tela quanto no PDF. Gravações **serializadas** (`_cronoSaveQ`).

**EcoReview (`sm.eco[]`).** O curso **EcoReview** (cadastrado no PO, detectado por `_cronoEcoCurso()` = vertical `ecoreview` OU nome casando `/eco.?review/i`) tem **biblioteca própria** (`_cronoEcoModulos()`: módulos direto do PO, temaId `eco__<slug>`, tempo = soma das durações das aulas). Botão discreto **"Adicionar EcoReview"** (entre revisão e aviso, estilo tracejado neutro) abre picker próprio (`#cg-eco-modal`, mostra duração; módulo some do box ao ser usado em QUALQUER semana — `_cronoEcoUsados()` — e volta se removido). Os módulos EcoReview **NÃO aparecem** nos pickers de tema/sobra/revisão: `_cronoModulosRaw` exclui o curso EcoReview **por id** (mesmo que ele esteja marcado como vertical `anestreview`). Exibição: **dentro do box "Se sobrar tempo"**, sob um sub-cabeçalho **ECOREVIEW** + `• nome do módulo`; o tempo dos módulos **soma no tempo de "se sobrar tempo" e no total da semana** (tela e PDF; no modo checkbox também ganham as 3 caixas).

**Export PDF** via jsPDF (A4 retrato, `_loadJsPDF`) — botão moderno (`.cg-export-btn`, gradiente) abre **menu com 2 formatos** (`_cronoExportMenu` → `#cg-export-modal`): `_cronoExportarPDF('normal')` e `_cronoExportarPDF('checkbox')`. Layout: cabeçalho escuro com **logo AnestReview embutido em base64** (const `CRONO_PDF_LOGO`, canto direito, fundo transparente) + título "CRONOGRAMA · nome" (encolhe se colidir com o logo) + "TREINAMENTO TSA ORAL · datas". **Box de datas no TOPO** (antes da Semana 1, compacto): "PROVA OFICIAL: X, Y ou Z", "REVISÃO PRÉ-PROVA: X, Y ou Z" + comentário, e "SIMULADOS:" (lista `nome: data` + linha com link clicável de escala `aros.anestreview.com.br`). Cada semana: header **"SEMANA N"** em maiúsculo/destacado com **"Tempo estimado total"** (prioritários+sobra); seções em **box de destaque com seta triangular vetorial** e tempo por seção — "TEMAS PRIORITÁRIOS PARA ESTUDO" (azul), "SE SOBRAR TEMPO ESTUDE" (cinza), "REVISÃO" (roxo); trilhas prefixadas "Trilha de Casos – …" + hot topics aninhados; **simulado** vira quadradinho laranja ("SIMULADO X / data / Confirme sua participação em <link>"). **Semana não quebra no meio da página** — `weekH(sm)` estima a altura e pula pra próxima página se não couber inteira (fallback: se for maior que uma página, parte). Modo **checkbox**: cada tema (prioritários+sobra) ganha 3 caixas de marcação alinhadas a colunas **ESTUDEI | NÃO ESTUDEI | REVISEI** (geometria `cbCols`/`CBW`; `weekH` é ciente do modo). **IA de montagem fica pra v2.**

**Divisão de módulo (overlay em `config/cronoConfig.divisoes`).** Um módulo do PO pode ser dividido em N partes SEM quebrar o import (a biblioteca é projeção de `poAulas` via `_cronoModulosRaw`; `_cronoModulos` aplica a divisão por cima). `divisoes[divKey]` (divKey = slug do módulo original) = `{cursoNome, modulo, partes:[{id,nome,aulaKeys[],aulasManuais:[{id,titulo,duracao:"MM:00"}]}]}`. Cada parte vira um **tema próprio** (temaId = `parte.id`), com aulas do PO atribuídas (por `_cronoAulaKey`) + **aulas manuais** (título+duração, pra produto ainda fora do PO). Aula não atribuída = "a classificar" (não some). UI: botão **✂ Dividir módulos** no cabeçalho da biblioteca → modal `#cg-div-modal`. Ao dividir, trilhas/hot topics do módulo inteiro NÃO migram pras partes. `config/cronoConfig` = `{ritmoCharsMin, multimidiaMin, trilhasOff[], trilhasTempoManual{}, temasOff[], divisoes{}, aulasRealoc{}}`.
