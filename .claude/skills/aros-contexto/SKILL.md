...
**3. Geração de Cronograma (`cronoGer`).** Coleção `cronoTurmas/{id}`: `{nome, dataInicio, dataFim, simulados:[{id,nome,ini,fim}], provas:[{id,nome,data}], revisaoPreProva:[{id,nome,data}], semanas:[{dias,temas[],revisoes[],nota}]}`. Semanas geradas de dataInicio+dias acumulados; datas/eventos calculados (`_cronoTurmaSemanas`). **Simulado** = intervalo (fim de semana); **prova** e **revisão pré-prova** = 1 dia por entrada, e ficam FORA das semanas — viram um **bloco separado no fim** (editor + PDF): "PROVA OFICIAL: X|Y|Z" e "REVISÃO PRÉ-PROVA: X|Y|Z" + nota (o cronograma termina no dia anterior à 1ª prova). Só **simulado** entra como badge de semana. Distribuição MANUAL: picker de temas em árvore por prioridade (tema não se repete entre semanas, exceto como revisão; revisão mostra "(visto na semana X)"). Semanas em dropdown (fechado por padrão) + Expandir todas. Export **PDF** via jsPDF (A4 retrato, cabeçalho escuro + accent; `_loadJsPDF`). **Distribuição por IA fica pra v2.** Gravações **serializadas** (`_cronoSaveQ`) + objeto mantido por referência — corrige corrida onchange-vs-clique que revertia nomes.
