...
**Barra de SAÚDE do módulo** (`_poSaudeDeAcoes`): `Π(1-(0.08+0.22·prioridade))` sobre as ações não dispensadas (curva amortecida — `_PO_SAUDE_MIN`/`_PO_SAUDE_VAR`; uma ação pesada não crava a barra). Aparece na **linha de cada módulo** (`_poProgBar`), **no box de análise** ("Saúde do módulo X%") e como **média no card do produto** (capa, `_poProdProgresso`). `null` = não analisado ("—"). Cor: ≥70 verde / ≥40 amarelo / <40 vermelho. (Coluna "Q: N" foi removida da linha do módulo.)
