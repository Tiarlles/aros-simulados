...
1. ✅ **Transcrição via Vimeo — ENTREGUE e no ar (2026-06-17).** Ver detalhe na seção "FONTE = Laravel" acima (`vimeoTranscricao` + `poTranscricoes/{vimeoId}` + botão "🎬 Puxar transcrições"). Falta só o Tiarlles rodar o lote das 436 aulas e **regenerar o token** do Vimeo (exposto no chat).
2. **IA real** de análise por módulo (EM FOCO agora): Cloud Function (padrão Dex/Sonnet) que recebe transcrição + avaliação + questões e devolve relatório priorizado; trocar o mock de `poAnalisarModulo`. (IA é CORS-bloqueada em localhost — testar no site publicado.)
3. ✅ **Sincronização automática Laravel→PO — ENTREGUE e no ar (2026-06-17).** Ver detalhe na seção "FONTE = Laravel" (`sincronizarLaravel` + `sincronizarLaravelAuto` semanal + botão "🔄 Sincronizar agora" com preview). Pra vigiar mais cursos: adicionar em `CURSOS_VIGIADOS`.
