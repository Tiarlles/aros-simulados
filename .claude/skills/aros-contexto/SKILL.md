...
- **Próximo:** **IA real** de análise por módulo (hoje mockada) usando a transcrição como fonte. ⚠️ Tokens colados no chat e orientados a **regenerar**: **Vimeo PAT** (`6eed…342f`, conta OFT-Review) + Client secret do app Vimeo. (O **Laravel token é permanente** — manter.)
...
- **Pedidos de alunos** por módulo: `poModQuestoes/{key}.pedidos:[]` (array; migrou de string), tabela no modal (cada linha salva). **4º balde "Outras"** nas questões (`_PO_MOD_TIPOS=['TEA','TSA','MEs','Outras']`).
- **Edital** por produto: `config/poConfig.editais[cursoId]` (campo único, botão "📋 Edital").
- **Pesos** da priorização: `config/poConfig.poPesos` + `PO_CRITERIOS` (7 critérios, sliders 0-50, botão "⚖️ Pesos"). **Design:** a IA pontuará cada critério 0-1; o código aplica os pesos → mexer peso re-ordena SEM chamar IA.
...
> **Onde paramos (2026-06-17, fim do dia):** tudo da Fase 1 publicado e funcionando em produção. O Tiarlles está **populando as questões módulo a módulo** (botão "🔄 Puxar questões da API" em cada Ponto) — em andamento, confirmou que está dando certo. Retoma amanhã. Próximo grande passo = a IA de análise (item 1), que precisa da **chave de API separada** da Anthropic (ainda não gerada).

1. **IA real de análise** (EM FOCO): Cloud Function (padrão Dex/**Sonnet**, **chave de API separada** a gerar) que, por módulo, recebe transcrição + avaliação + ano + status + apostila-status + questões (já na API) + pedidos + edital → devolve **ações com nota 0-1 por critério**; o código aplica `poPesos`. Produto = agrega módulos já rodados (incremental, com "re-rodar todos") → linha do tempo. Trocar o mock `poAnalisarModulo`/`poAnalisarProduto`. (IA CORS-bloqueada em localhost — testar publicado.)
