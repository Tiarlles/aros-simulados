...
- Firebase funciona normal (conecta da internet independente da origem). No iPhone o Google Auth também exige domínio autorizado — pra testar login use o navegador do Mac via `localhost`.
...
- Reusa `alunoRowHTML(s, exp, extraAfterName)` — assinatura ganhou 3º param opcional (refator pra evitar regex frágil em nomes com `<`/`>`, ver Bug 6 abaixo).
...
- **User info card** (avatar+nome+role) movido pro **topo** da sidebar (wrapper `.co-sidebar-head` com leve gradiente azul). Estava no `co-sidebar-foot`. Logout button continua no foot.
- **Grupos da sidebar fechados por padrão na primeira visita**: `renderSidebar` trata `localStorage.getItem('aros-sidebar-collapsed')==null` como "tudo colapsado" (popula com todos os `group.id` e `subgroup.id`).
...
- Helper `userTabs(u)`: tipo `adm` → `ALL_TABS`; senão → `u.permissoes` (abas que VÊ).
- **Modelo de permissão por aba REVISADO em 2026-05-30** (ver-vs-admin separados): `permissoes[]` = abas que a pessoa VÊ; novo `adminTabs[]` = abas onde tem poderes de ADMIN (o slider "ADM" no cadastro). `_isAdminEm(tabId)` = `tipo==='adm'` OU `adminTabs.includes(tabId)`, **com fallback legado**: `if(!Array.isArray(u.adminTabs)) return permissoes.includes(tabId)` (usuário antigo sem `adminTabs` mantém comportamento anterior — ninguém perde acesso até ser re-salvo). Helper `_userIsAdminTab(u,tab)` pré-marca os sliders ao editar (mesma lógica de fallback + caso `gerenciarCatalogo→produtos`).
- **`ADMIN_LAYER_TABS`** (Set hardcoded, ~11371) = `['financeiro','orcamento','senhasAcessos','cruzarLista','produtos']` — únicas abas que ganham slider Admin (as que têm ações de admin separadas via `_isAdminEm`). Nas demais, ver=acesso total (sem slider). **⚠️ Ao adicionar ações de admin numa aba NOVA no futuro, incluir o id dela em `ADMIN_LAYER_TABS`** pro slider aparecer. NÃO é auto-detectado.
- **`gerenciarCatalogo` aposentado** → virou o slider Admin da aba `produtos`. `_proCanEdit` = `_isAdminEm('produtos') || permissoes.includes('gerenciarCatalogo')` (2º termo = fallback legado). `EXTRA_PERMS`/`_isExtraPerm` ainda existem (userTabs filtra extras de docs antigos).
- **`STRICT_ADMIN_TABS` agora vazio** (`auditoria` removida em 2026-05-30 → delegável). Auditoria ainda mostra badge ADM (nenhum preset não-adm a inclui) mas PODE ser concedida marcando a aba. **Cuidado:** se incluir `auditoria` num preset não-adm, todos daquele tipo veem o log.
- **UI do cadastro (modal `#modal-user`)**: grupos em `<details class="mu-pgroup">` (fechados por padrão) com checkbox de header que faz select-all (`_muToggleGroup`/`_muSyncGroupChk`, estado indeterminate). Cada aba = `.mu-row` com `.mu-perm` (ver) + slider `.mu-admin`/`.mu-switch` (só nas ADMIN_LAYER_TABS, com texto "ADM" translúcido). `saveUser` lê `#mu-perms-grid .mu-perm:checked`→permissoes e `#mu-perms-grid .mu-admin:checked`→adminTabs (ambos `[]` pra adm).
...
- Lista canônica em `AUDIT_ACTIONS` (linha ~4833): `LOGIN_CUSTOM`, `LOGIN_FIREBASE`, `LOGIN_GOOGLE`, `LOGOUT`, `SIMULADO_CRIADO`, `SIMULADO_EDITADO`, `NOTA_LANCADA`, `NOTA_RESETADA`, `ALUNO_ADICIONADO`, `ALUNO_REMOVIDO`, `ALUNO_STATUS_ALTERADO`, `ALUNO_MOVIDO`, `ALUNO_SWAP`, `PRESENCA_TOGGLE`, `MENTORIA_CRIADA`, `MENTORIA_EDITADA`, `MENTORIA_REMOVIDA`, `SOLICITACAO_EXTRA_EFETIVADA`, `SOLICITACAO_EXTRA_EDITADA`, `USUARIO_CRIADO`, `USUARIO_EDITADO`, `USUARIO_DESATIVADO`, `COMUNICACAO_POST_CRIADO`, `COMUNICACAO_POST_EDITADO`, `COMUNICACAO_POST_DELETADO`, `CONFIG_ALTERADA`, `CONTROLE_TOGGLE`, `PROVA_STATUS_TOGGLE`, `CK_CASO_SALVO`, `CK_BLOCO_FINALIZADO`, `CK_CASO_RESETADO`, `CK_PERGUNTA_RESETADA`, `CK_ALUNO_RESETADO`, `AUTO_AUSENTES_POS_PRAZO`.
...
**Posicionamento da imagem** (`imagemPos`): `baixo` | `direita` | `grande` | `cima` | `esquerda` (atualizado na sessão de 2026-05-29 (continuação)). Container limitado a `max-height:50vh` com `overflow:hidden` pra evitar overlap com texto **— exceto em Acima/Abaixo/Tela cheia, onde o cap foi removido pra deixar o slider de tamanho de texto definir o espaço; `.pbody` vira `flex:1 1 0`**.

**Atualizado na sessão de 2026-05-29 (continuação)** — campos adicionais por slide:
- `imagemSize` (20-200, default 100) — `transform:scale(var(--img-size))`; >100% crop via `overflow:hidden`.
- `textScaleEnun` / `textScalePerg` (70-150, default 100) — sliders independentes via CSS custom props `--txt-enun` / `--txt-perg`. Fallback chain pro `textScale` legacy.
- Posições novas `cima` e `esquerda` (além de `baixo`/`direita`/`grande`).
- `_slideMigrar` adiciona defaults pra slides antigos.
...
**Editor de slides** (modal "🎬 Slides") — **redesign Apple-like dark glass na sessão de 2026-05-29 (continuação)**:
- Cards colapsáveis com badge SLIDE N, tipo colorido, snippet, botões de ação. Glass effect `backdrop-filter:blur(14px) saturate(140%)`, gradientes accent.
- **Drag-and-drop nativo pelo handle `⠿`** (substitui setas ▲▼). Drop zones com feedback azul; re-mapeia `_slidesOpen` após drop.
- Header minimalista: `⠿` + caret + "SLIDE N" + espaçador + ações. Modal `data-no-backdrop-close`.
- Toolbar global: botão único `⊞ Expandir todos ↔ ⊟ Recolher todos`. **"🖼 Adicionar slide pronto" REMOVIDO da toolbar global** (continua por slide).
- Botão por slide: "🖼 Slide pronto" / "↩ Slide automático" + "👁 Preview".
- Mini editor rich text (`_rtEditor`): **B / I / U + 4 botões de alinhamento SVG inline (Word-style)**. Removidos `A+`/`A-` (slider de texto cobre). HTML sanitizado por `_sanitizeSlideHTML` (whitelist agora aceita `text-align`).
- **Position picker visual** pra perguntas: 5 mini-thumbnails (Acima · Abaixo · Esquerda · Direita · Tela cheia). Substitui dropdown legado.
- **3 sliders por slide**: tamanho de imagem (20-200%) + tamanho do enunciado (70-150%) + tamanho da pergunta (70-150%). Sliders preservam foco durante drag (DOM update direto + rAF debounce).
- Drop zone de upload compacta (108px → 42px). Limite upload subiu 15MB → 50MB.
...
- **`👁 Preview` agora abre janela real 1280×800** (`?modo=preview&slideId=X`) com live update via `BroadcastChannel('aros-slides-edit')` + rAF debounce. Badge "AO VIVO" verde no header. Filtro por `slideId` mostra 1 slide.
...
- **Atualizado na sessão de 2026-05-29 (continuação)** — bug "clique perdido em Próximo/Anterior" também consertado no painel do caso (`ck-proj-{ci}`): o `tickInt` rodava `_atualizaPainelProj` a cada 500ms, recriando innerHTML e destruindo o botão entre `mousedown` e `mouseup`. Fix: tick atualiza só `textContent` do timer in-place; painel só re-renderiza quando idx muda. Lock anti-double-click `st._navLocked` em `projecaoNav` (450ms min) com UI `cursor:wait`+`opacity:.55`+`title=Aguarde…`.
- **Barra de desenho movida pro topo (2026-05-29 cont.)**: caneta + paleta + borracha + limpar saíram do canto inferior direito e viraram `<span>` inline dentro do `#proj-timer`, entre cronômetro e botões de nav. Visível pra prof E aluno (PIP do prof carrega iframe como `role=aluno`). Separador via `border-left:1px solid #333`.
...
**Upload de imagens (slides)**: `simulados/{simId}/slides/{casoId}/{timestamp}_{filename}`. Limite **50MB** (subiu de 15MB na sessão de 2026-05-29 (continuação)).
...
**Vídeo da Live (YouTube/StreamYard) na visão aluno (2026-05-30):**
- Config em `recursosConfig/settings.liveVideo = {videoId, url, titulo, ativo, updatedAt}`. Admin cadastra via botão **🔴 Vídeo da Live** na toolbar da aba Gestão de Recursos (`rec-gestao-acts`) → modal `#modal-rec-live` (dark glass, `data-no-backdrop-close`, com pré-visualização ao vivo). Funções `openRecLiveModal`/`recLivePreview`/`saveRecLive`; helper `_ytExtractId(url)` extrai o ID de 11 chars de watch/live/embed/youtu.be/ID cru.
- **Embed:** `https://www.youtube.com/embed/{videoId}?rel=0&autoplay=1&mute=1&playsinline=1` (autoplay exige `mute=1`), container 16:9 responsivo.
- **Regra de exibição** (só quando `ativo && videoId`): aparece em 3 lugares — (1) abaixo do grid de provas em `renderRecAluno`; (2) **substitui** o card "Nenhuma prova ativa" quando não há prova publicada (`if(_liveHtml){el.innerHTML=_liveHtml;return;}`); (3) **mini-player compacto colapsável** (`<details class="rec-live-mini" open>`, ≤420px) no topo da aba da prova em `_renderAlunoProvaUI`. `renderAlunoProva` carrega `loadRecConfig()` em paralelo com as questões pra funcionar em deep-link direto.
- **Card vazio redesenhado** (`.rec-empty`): dark glass com halo animado, eyebrow mono, `.rec-empty-art` (150px) que aceita `<img>` full-art ou o ícone fallback 📭. Pendência: imagem gerada por IA (1536×1024 ou 1792×1024, texto embutido) pra preencher o box inteiro — quando o usuário enviar, trocar o ícone por `<img>`.
...
**Modal de usuário — redesign + senha opcional + convite de acesso (2026-05-30):**
- Dark glass (Space Grotesk + JetBrains Mono labels), `data-no-backdrop-close`, título "Cadastro de Usuário"/"Editar Usuário". Textos enxugados (várias descrições removidas).
- **Senha agora é OPCIONAL** (`saveUser`): em criação sem senha → placeholder não-adivinhável (`'_acesso_'+rand`, desativa login clássico, pessoa usa convite/Google); em edição sem senha → mantém a atual (`_userBefore.senha`). Validação só exige nome de usuário.
- **Convite de acesso (qualquer tipo, não só prof)**: checkbox `#mu-invite-acesso` (marcado default em criação). Ao salvar, `_onbSendInvite(user,email)` → `_criarAcessoConvite(email)` → **Cloud Function `criarAcessoConvite`** (cria conta no Firebase Auth se não existe + `generatePasswordResetLink` → link de DEFINIR senha) → EmailJS `template_rruper4` com var `setup_link`. A pessoa clica no link e cria a própria senha (ou usa Google). Endpoint: `https://us-central1-simulados-confirmacao.cloudfunctions.net/criarAcessoConvite`. CF valida que o chamador é admin (usuarios tipo=adm/role=admin ou OWNER_EMAILS). **Template EmailJS precisa usar `{{setup_link}}`** no "AQUI".
- Validação de campos com feedback visível: helper `_muErr(msg,focus)` foca/destaca (`.mu-invalid`, shake) o campo que falta + scroll. Reset defensivo do botão/destaques ao abrir o modal.
- **Cloud Function nova**: `cloud-function-hotmart/criar-acesso.js` → `criarAcessoConvite` (Admin SDK: `createUser` + `generatePasswordResetLink`). Registrada no index.js. **Precisa de deploy** pra o convite funcionar (sem deploy, o botão dá erro de conexão).
...
**Checklist — ajustes da lista de alunos + projeção (2026-05-31):**
- **Barra fixa de projeção no topo REMOVIDA**: `_atualizaBarraFixa()` virou só "remove-residual" (early return que apaga `#proj-fixed-bar` + tira `body.has-proj-bar`). Os controles ficam só no box do caso (`renderProjPanel`) — evita 2 cronômetros na tela. Código antigo da barra fica inerte após o `return`.
- **Um cronômetro por vez**: `iniciarProjecao(ci)` encerra automaticamente qualquer OUTRA projeção ativa (`encerrarProjecao(o)` pra todo `o!==ci` em `_projAtivo`) antes de criar a nova — feito DEPOIS das validações. Toast leve "⏱️ Cronômetro do caso anterior encerrado". Resolveu aplicadores deixando 2-3 rodando.
- **Lista de alunos (`renderCkStudents`)**: cada linha ganhou 2 selos clicáveis logo após o nome — `📋 Criar` e `🎙️ Oral Online` — mostrando Finalizado/Em andamento/Pendente (mapeia green/yellow/red de `getBlocoStatus`). Clicar leva direto ao bloco do aluno via `ckGoBloco(studentId,bloco)` = `openCkFill` + `openCkBloco`. Removido o badge geral de status (redundante) — mantido só o botão "🔖 Gerar Feedback" (aparece quando os 2 blocos finalizam). Borda da linha neutra; **fundo da linha** comunica status: ambos pendentes=neutro, misto/andamento=amarelo leve, ambos finalizados=verde leve.
- **Filtros por bloco**: abaixo da busca, "Filtrar bloco:" com 2 dropdowns (`ck-filter-criar`/`ck-filter-oral`, valores Todos/pendente/andamento/finalizado) → `ckSetBlocoFilter(bloco,val)` grava em `S.ckFiltroBloco`. Filtro combinado (nome + 2 blocos) aplicado DENTRO de `renderCkStudents` via `_ckApplyFilters` (vale em TODAS as re-renders, inclusive live updates). Helper `_ckStatusBloco(s,bloco)`. Chip "✕ Limpar" (`ckLimparFiltroBloco`). Reseta ao trocar de simulado (`loadCkSim`).
...
### Sistema de Recursos — segunda maratona UX/design (2026-05-28 noite)

Continuação do mesmo dia, focada em polimento UX/design Apple-like dark glass + funcionalidades novas. Tudo entregue + deployado.

**Modal de contestar do aluno:**
- Backdrop bloqueado (`data-no-backdrop-close`).
- Redesign Apple: eyebrow mono, seções nomeadas, halo azul no foco, padrões `.mct-*`.
- Textarea de motivo: **texto justificado por padrão** (CSS `text-align:justify`).
- **Bibliografia obrigatória** (validação no envio); selo "obrigatório".
- Novo campo opcional **Capítulo** ao lado de Página.
- **Pre-modal de aviso "Já existe parecer"** (`modal-contest-warn`) com SIM/NÃO ANTES de abrir o form de contestação (alertaParecer removido de dentro do modal).
- **Modal customizado "Contestação Enviada"** (`modal-contest-ok`) — checkmark animado, anel girando, halo pulsante; substitui `alert()`.
- **Botão "Contestar Gabarito"** redesenhado: gradient azul, brilho varrendo no hover, seta deslizando, sombra glow.
- **Bloqueio quando há contestação pendente de QUALQUER aluno**: botão vira box "📨 Contestação em análise · aguarde a resposta antes de enviar nova".
- **Defesa em profundidade**: `alunoAbrirContestar` e `enviarContestacao` revalidam o bloqueio.

**Visibilidade pública de contestações:**
- `S.recursos.alunoState.todasContestacoes` (novo) — load PARALELO de todas as contestações no `renderAlunoProva`.
- Função `_alunoContestacoesPublicas(lista, emailNorm, qId, questao)` renderiza box **collapsible (dropdown)** "📨 CONTESTAÇÕES RECEBIDAS (N)" com pendentes/respondidas no header.
- Cada contestação aparece anônima (sem email/data); contestação do próprio aluno ganha badge "VOCÊ".
- Estado de expansão em `S.recursos._alunoContestExp` (Set).
- **Botão "Ver Resposta"** (`alunoVerResposta(qId, cId)`) em cada respondida → abre `modal-cont-resp` com motivo original + veredito (CABE/NÃO CABE) + parecer da AnestReview + bibliografia + imagem.
- `respostaSnapshot` agora inclui `fonte, fonteCustom, capitulo, pagina` (além de parecer/argumento/imagemUrl/profNome/respondidoEm).

**Aluno · Home /recursos:**
- Hero compacto **dark glass** com grid HUD (linhas 32px com máscara radial), 4 brackets nas quinas, blob roxo flutuante, status pill com dot pulsante.
- Título: **"Sistema de Recursos AnestReview"** com gradient azul→roxo na marca.
- Cards de prova com cursor luminoso seguindo mouse, badge "Recursos até XXX" destacado (pílula azul com dot pulsante), CTA gradient.
- **Cards com prazo encerrado**: opacity .78 + badge vermelho "🔒 PRAZO ENCERRADO" + barra lateral vermelha.
- Modal de identificação por email com card glass, ícone circular pulsante, badge da prova, atalho Enter.
- Empty state refinado (ícone circular emoldurado).

**Aluno · prova aberta:**
- Header compacto (~60% menor que antes) com botão "Voltar" como pílula colorida acima do box (saiu de dentro).
- **Título = TIPO · ANO** (ex: "ME1 · 2026") em 34px peso 800 com text gradient da cor do tipo. Removido o campo "Nome da prova" — auto-gerado.
- **Contador regressivo** virou pílula slim dark glass numa linha: **"● Faltam 5d 3h 2m para encerrar o prazo de recursos"**. Dot pulsante na cor do status (verde→amarelo→laranja→vermelho).
- Quando encerra: **bloqueia botão Contestar em todas as questões** + força re-render + alert/inline error no submit.

**Stats bar (abaixo do mapa de questões):**
- 3 cards compactos dark glass: **Análises realizadas** (verde · contestações respondidas) · **Recursos possíveis** (laranja · `cabe-recurso` total · subtexto "questões laranjas no mapa") · **Tempo médio de resposta** (azul · `respondidoEm - criadoEm` médio).
- Ícone chip 20×20 + label colorida em uma linha; número em 24px (era 38px — reduzido por pedido).
- Linha luminosa horizontal no rodapé + halo radial flutuante no canto.
- Função: `_renderAlunoStats(questoes)`.

**Mapa de questões:**
- Continua igual visualmente.
- Agora tem **legenda dinâmica embaixo** (só status presentes) com pílulas coloridas + dot glow + contador.

**ID externo das questões:**
- Novo campo opcional `idExterno` (max 8 chars, uppercase) no editor inline da questão (`iq-idExterno-{qId}`).
- Exibido pro aluno na linha do card de questão antes do status: **`[1] ID Anest XXXX | STATUS`** (pílula azul + separador `|`).
- Persiste em `provas/{provaId}/questoes/{qId}.idExterno`.

**Visão Coord · Gestão de Recursos:**
- Cards clicáveis (clique no box = abre parecer). Removido botão "👁 Visualizar". Removido snippet do motivo na linha (deixou só metadados).
- **Botão "📋 Instruções de Envio"** na toolbar (dark glass com chip de ícone + título + subtítulo mono "TEXTO EXIBIDO PARA O ALUNO"). Clica → abre `modal-instrucoes`.
- **Aba "Instruções & Config" REMOVIDA** (HTML, menu, switchAdminTab, ADMIN_ONLY_TABS, `renderRecConfig` deletado).
- **Modal de instruções**: editor rich text com toolbar full (B/I/U/S, listas, alinhamento, 7 cores texto + 5 fundos, limpar). Texto **justificado por padrão** (CSS força no editor e no view). Salvo em `recursosConfig/settings.instrucoes` (mesmo campo legado — texto puro vira parágrafos automaticamente).
- **Modal `modal-instr-view`** (aluno) renderiza HTML formatado quando clica em "📖 Como enviar o recurso oficial".
- **Função `mensagemTopo` removida** do config (não tem mais box; o campo `mensagemInicial` da prova ainda é usado como fallback no hero).
- **`loadAllContestacoes` paralelizado**: Promise.all em provas + questões + contestações (era N+1+M sequencial → cai de minutos pra segundos).

**Modal de parecer (coord):**
- Adicionados campos opcionais **Bibliografia + Capítulo + Página** (caixinha "📖 Referência Bibliográfica" dark accent ABAIXO do editor de texto, não em cima).
- Persiste em `parecerFonte, parecerFonteCustom, parecerCapitulo, parecerPagina` na raiz da questão E em cada `sugestoes[].fonte/capitulo/pagina`.
- **"Assumir análise" SEM modal de reidentificação** se prof já estiver logado — chama `_confirmarAssumirAnalise(profLogado)` direto. Admin/coord sem prof vinculado ainda vê o dropdown como fallback.
- Refatorado `_confirmarAssumirAnalise` pra `throw Error` ao invés de tocar DOM — caller decide se mostra alert ou inline.

**Cadastro de prova — modal redesenhado:**
- Glass dark Apple-like com eyebrow "PLATAFORMA DE RECURSOS".
- **Tipo virou grid de tiles** (110px+) com **bolinhas tech** (12px com glow + inset 3D) — substituiu o dropdown emoji.
- Tile selecionado: borda colorida + halo + sombra + bolinha pulsando.
- **Cadastro de tipos custom**: último tile é "+ Novo" → form inline com sigla (max 6) + paleta 10 cores. Salvo em `recursosConfig/settings.tiposCustom = [{key,cor}]`. `_mpvEnsureCustomCores()` injeta no `_MENTORIA_CORES` em runtime. Bloqueia delete se houver prova usando o tipo.
- **`MENTORIA_TIPOS` substituído por `_mpvAllTipos()`** no editor de bibliografia (built-in + custom).
- **Toggle Rascunho/Publicada** substitui dropdown de status: switch com thumb gradient deslizando (cinza→verde), dot pulsante no ativo. Padrão: **Rascunho**.
- **Status "rascunho"** não aparece no front (`/recursos` filtra com `_provaPublicada(p)` — compat com legado `'ativa'` que ainda conta como publicada).
- **Campo "Nome da prova" REMOVIDO** — auto-gerado como `${tipo} ${ano}`.
- **Linha de status no card do back**: "PUBLICADA" / "RASCUNHO" (em vez de ATIVA/ENCERRADA).
- **Removido botão "+ Nova questão"** (cadastro só via import).

**Cabeçalho da prova ao abrir (back):**
- Título grande na cor do tipo + contagem de questões em mono pequeno (sem chip redundante "ME1 · 2026" + texto repetido).

**Botões de importar redesenhados:**
- Pílulas grandes com gradient (azul→roxo pra questões, laranja→amarelo pra gabarito).
- Ícone chip semi-transparente + título "Importar Questões" + subtítulo mono "PDF OU COLAR TEXTO" + chevron que rotaciona.
- Brilho varrendo no hover, sombra colorida growa, translate -2px.

**Modais de importação (3) redesenhados:**
- Estilo compartilhado `.mim-*` Apple dark glass (radius 22px, blur 40px, shadow profunda).
- Eyebrow mono pulsante com cor própria por modal (azul/laranja/roxo).
- Drop zones com ícone chip glow + CTA gradient.
- Textareas dark glass com halo no foco.
- Caixa de regras com background colorido + `<code>` em pílulas accent.
- Foot: botão "Cancelar" outline + "Importar" gradient com sombra.

**Bibliografia:**
- **Campo Descrição REMOVIDO** (só Nome + Tipos).
- Tipos viraram **dropdown** (era checkboxes inline): trigger button "Selecione os tipos / Todos os tipos / 3 selecionados", panel glass dark com max-height 320px scroll.
- **Topo do panel: "✓ TODOS OS TIPOS"** — checkbox que marca/desmarca todos os individuais. Auto-marcado quando todos individuais estão marcados.
- Cada tipo é linha com checkbox + bolinha colorida tech + label mono.
- **Inclui tipos custom** automaticamente (`_mpvAllTipos()`).
- Panel com `background:#0d1117 !important; backdrop-filter:none !important; isolation:isolate` — totalmente opaco (resolve bug de transparência reportado).
- **Removida categoria "GERAL"** do filtro de topo (só Todos + tipos).
- **Listagem colapsável**: pílula azul "Provas vinculadas · N ▾" que expande caixa dashed com tags.
- Bibliografias antigas com `tipos=[]` (legado "geral") agora aparecem em qualquer filtro de tipo + mostram pílula "✓ TODOS OS TIPOS".

**Mudanças no modelo de dados (Firestore):**
- `provas/{provaId}`:
  - `status: 'rascunho' | 'publicada'` (novo, default `rascunho`). Compat: `'ativa'` legado conta como publicada.
  - `nome` auto-gerado como `"${tipo} ${ano}"` (campo continua existindo, só não é mais editável).
- `provas/{provaId}/questoes/{qId}`:
  - `idExterno: string` (opcional, max 8 chars, uppercase) — exibido pro aluno antes do status.
  - `parecerFonte, parecerFonteCustom, parecerCapitulo, parecerPagina` (raiz, espelho da sugestão mais recente).
  - `sugestoes[].fonte, fonteCustom, capitulo, pagina` (cada sugestão).
- `provas/{provaId}/questoes/{qId}/contestacoes/{cId}`:
  - `capitulo: string` (novo, opcional) — cadastrado pelo aluno.
  - `respostaSnapshot.fonte, fonteCustom, capitulo, pagina` (snapshot do parecer no momento da resposta).
- `recursosConfig/settings`:
  - `tiposCustom: [{key, cor}]` (novo) — tipos de prova customizados.
  - `instrucoes: string (HTML)` (existia; agora aceita HTML rico em vez de texto puro).
  - `mensagemTopo` (mantido pra compat mas não tem mais UI de edição — pode ser removido em sessão futura).

**Pendente registrado (não entregue hoje) — ENTREGUE em 2026-05-29:**
- ~~**Refinar template de email do parecer**~~ — entregue na sessão de 2026-05-29 via `template_recurso_parecer` (branded MEDREVIEW, light mode Tahoma) com bibliografia/imagem/argumento/link contextualizado. Ver seção "📧 Template dedicado de parecer".

**Commit da sessão**: `b2bf814` "Sistema de Recursos: maratona de UX/design (sessão 2026-05-28)" — 2014 inserções, 401 deleções no `index.html`. Deploy via push pra `main` (GitHub Pages atualiza `aros.anestreview.com.br`).
...
- `provas/{provaId}`: `nome` string não vazia + `tipo is string && tipo.size() > 0` (afrouxado em 2026-05-29 pra suportar `tiposCustom` de `recursosConfig.settings`) + `status in ['rascunho','publicada','ativa','encerrada']` (afrouxado em 2026-05-29 pra liberar criação de prova em rascunho/publicada — `ativa`/`encerrada` mantidos por compat com legado).
...
### Editor de Template do Checklist (refatorado 2026-05-14/15, redesign Apple-like dark glass em 2026-05-29 cont.)
...
**Redesign Apple-like dark glass (2026-05-29 cont.) — caixas de caso e pergunta:**
- Classes CSS novas (área linha 800, depois de `.rt-ed:focus`): `.tpl-caso`, `.tpl-caso-head`, `.tpl-caso-num`, `.tpl-caso-input`, `.tpl-caso-body`, `.tpl-perg`, `.tpl-perg-head`, `.tpl-perg-num`, `.tpl-perg-input`, `.tpl-perg-body`, `.tpl-caret`, `.tpl-perg-caret`, `.tpl-section-label`, `.tpl-textarea`, `.tpl-btn-action` (+ `.del` variante vermelha).
- `backdrop-filter:blur(14px) saturate(140%)`, borda translúcida com `color-mix`, glow no hover.
- **Faixa lateral colorida** (3px → 5px no hover/open) via CSS custom prop `--tpl-cor` (azul Criar, laranja Oral).
- **Header inteiro clicável** (tanto caso quanto pergunta) pra abrir/recolher. Input/textarea/button mantêm comportamento via check `closest()`.
- Caret animado `rotate(90deg)` com `cubic-bezier(.4,0,.2,1)`. Badge "CASO N" / "P{n}" pill JetBrains Mono.
- Removida contagem de perguntas do header do caso.
- Botão "🎬 Slides" renomeado pra **"🎬 Editar Slides"** com contador entre parênteses.
- **Botão novo "👁 Pré-visualizar"** antes do Editar Slides — helper `previewCasoSlides(bloco, ci)` abre janela 1280×800 com todos os slides do caso (gera padrão via `gerarSlidesPadrao` se ainda não tiver sido customizado).
- Label "📝 Enunciado do caso (opcional)" → **"ENUNCIADO"** (uppercase, JetBrains Mono, letter-spacing — padrão `.tpl-section-label`). Textarea rows 3 → 6.
...
- Aplica IA (**Claude Sonnet 4.6 via Cloud Function** — ver abaixo) em cada feedback de caso via `applyAIPrompt` (silent fallback se sem login Firebase).
- Abre view `ck-preview` populando dados via `abrirPreviewV2(data)`.

#### IA dos feedbacks — migrada de Gemini pra Claude (2026-05-30)

Antes usava Google Gemini 2.5 Flash com chave client-side em `config/settings.aiKey`. **Migrado pra Claude Sonnet 4.6 via Cloud Function** (chave no servidor, nunca exposta no cliente):
- **Cloud Function `gerarFeedbackIA`** (`cloud-function-hotmart/feedback-ia.js`, espelha o padrão do Dex): POST `{prompt, content, maxTokens, mode}` + `Authorization: Bearer <Firebase ID token>`. `mode`: `'revisar'` (feedback por caso), `'geral'` (consolida contribuições), `'test'` (ping). Default model `claude-sonnet-4-6`. Endpoint: `https://us-central1-simulados-confirmacao.cloudfunctions.net/gerarFeedbackIA`.
- **Chave dedicada** `ANTHROPIC_API_KEY_FEEDBACK` no `.env` (separada da `ANTHROPIC_API_KEY` do Dex de propósito — pra estimar custo isolado no console Anthropic). Sem fallback: falta da var → erro claro.
- **Frontend**: helper `_callFeedbackIA({prompt,content,maxTokens,mode})` (pega token via `window._fbAuth.auth.currentUser.getIdToken()`). Usado por `applyAIPrompt`, `gerarFeedbackGeralIA`, `testAiConnection`. **Exige Firebase Auth** (login custom só-senha não acessa). `S.cfg.aiKey` aposentado (Gemini removido).
- **Modal `#modal-prompt-ia`** (config dos prompts, admin): redesign dark glass (Space Grotesk + JetBrains Mono), sem campo de chave (fica no servidor), `data-no-backdrop-close`. Botão **⤢ Editar em tela cheia** abre overlay genérico `#modal-prompt-editor`.
- **Editor genérico `#modal-prompt-editor`** (z-index 100000, `data-no-backdrop-close`): `openPromptEditor(taId, nome, {persist, simId})`. `persist`: `'aiprompts'` (salva `config/aiPrompt` via `_saveAiPromptsToDB`), `'fgfinal'` (salva consolidado via `salvarFeedbackGeralFinal(simId)`), ou null (só aplica o texto no campo de origem). `savePromptEditorAndClose` reflete no textarea de origem + persiste conforme o modo + toast. Reusável em qualquer textarea.
- **Modal `#modal-feedback-geral`** (contribuição do prof) e **`#modal-fg-admin`** (Configurar Feedback Geral): mesmo dark glass + editor em tela cheia + `data-no-backdrop-close`. No admin, cada contribuição é um `<details>` (dropdown: prof+data no resumo, texto+remover no corpo) e o botão de IA foi renomeado pra **"✨ Aplicar formatação com IA"** (id `btn-gerar-fg-ia`, fn `gerarFeedbackGeralIA`).
...
**Status**: **ENTREGUE em 2026-05-29.** Template `template_recurso_parecer` (service `service_exyoa4r`) criado e em uso. Branded MEDREVIEW, light mode Tahoma. Vars enviadas via `sendParecerEmail(emailAluno, ctx, data)`: `to_email, nome_aluno, prova_nome, questao_numero, parecer_label, parecer_cor_bg, parecer_cor_fg, argumento` (texto puro com `\n` via `_htmlParaTexto` + `white-space:pre-wrap` no template), `bibliografia_html` (HTML montado no JS, vazio se sem dados), `imagem_html` (idem), `link_recurso`, `prof_nome`. Template usa **triple braces** `{{{bibliografia_html}}}` e `{{{imagem_html}}}` (EmailJS suporta) pra renderizar HTML. Deep-link `/recursos?provaRec=X&qRec=Y` (lido por `goRecursosAluno`, `_pendingQScroll` consumido em `recAluConfirmarEmail` se aluno precisar identificar email primeiro → `alunoIrParaQuestao`). Helpers: `_resolveFonteNome(fonte, fonteCustom)`, `_htmlParaTexto(html)`. Arquivo de referência do template: `email-template-recurso.html` na raiz do repo.

## Histórico recente (resumo cronológico)

### Sessão 2026-05-29 (continuação) — redesign editor de slides + checklist template

Sessão dupla longa de redesign visual (Apple-like dark glass) do **editor de slides** e do **editor de template do checklist**, com correções de bugs críticos na projeção live. Commits: `f2d42ff`, `8e1ea49` (slides + projeção) e `5efb26d` (template do checklist).

**Parte 1 — Editor de slides (`#modal-slides`)**

Redesign visual completo:
- Cards com **glass effect** (`backdrop-filter:blur(14px) saturate(140%)`), gradientes accent, transitions `cubic-bezier(.4,0,.2,1)`, pill badges colorpíndo por bloco.
- **Drag handle `⠿`** com drop zones e feedback visual azul (substitui setas ▲▼ legacy). Drag-and-drop nativo: cada card draggable pelo handle; re-mapeia `_slidesOpen` para os novos índices após drop; visual usa borda accent + translateY no card alvo.
- Header minimalista: `⠿` + caret `▶` + "SLIDE N" + espaçador + ações.
- Toolbar global limpa: botão único toggle `⊞ Expandir todos ↔ ⊟ Recolher todos` (substituiu 2 botões). Botão "🖼 Adicionar slide pronto" **REMOVIDO** da toolbar global — continua disponível por slide individual.
- Botões com label: "👁 Preview" e "🖼 Slide pronto" / "↩ Slide automático".
- Modal **não fecha ao clicar fora** (`data-no-backdrop-close`).
- Upload drop zone compacta (108px → 42px altura, layout horizontal). Limite de upload subiu **15MB → 50MB** em `slideUploadImg`.

Controles novos por slide (perguntas):
- **Position picker visual**: 5 mini-thumbnails (Acima · Abaixo · Esquerda · Direita · Tela cheia). Substitui o dropdown que tinha só 3 opções. Adiciona posições `'cima'` e `'esquerda'` ao set legado (`baixo`, `direita`, `grande`).
- **Slider tamanho de imagem** (20-200%) via CSS custom prop `transform: scale(var(--img-size, 1))`. Acima de 100% aplica zoom + crop (overflow:hidden no `.pbody`).
- **2 sliders independentes de texto** (📝 Enunciado · 📝 Pergunta), 70-150% via `--txt-enun` / `--txt-perg`. Backward compat com `textScale` legacy via fallback chain.
- Sliders **preservam foco durante drag** (DOM update direto + rAF debounce no broadcast) — fix do glitch "imagem some no meio do arraste".
- Removidos botões `A+` / `A-` (slider de texto cobre).

Novos campos do slide (em `caso.slides[i]`):
- `imagemSize` (número 20-200, default 100)
- `textScaleEnun` (número 70-150, default 100, fallback `textScale`)
- `textScalePerg` (número 70-150, default 100, fallback `textScale`)
- `_slideMigrar` adiciona defaults pra slides antigos.

Layouts dinâmicos (verticais):
- Removido o cap fixo `max-height:50vh` nos layouts Acima/Abaixo/Tela cheia.
- `.pbody` usa `flex:1 1 0` → ocupa todo espaço restante após o texto. Texto pequeno → imagem grande; texto grande → imagem encolhe.

Rich text editor (mini editor por slide):
- **4 botões de alinhamento** estilo Word com **SVG inline** (linhas variáveis representando o layout).
- Default `text-align: justify` + `hyphens: none` em 10 lugares de render do slide.
- `_sanitizeSlideHTML` aceita `text-align` no whitelist do atributo style.

Pré-visualização ao vivo (refatorada):
- Removido popup pequeno (era 540px, ficava desconfigurado).
- `👁 Preview` agora abre **janela real 1280×800** com URL `?modo=preview&slideId=X`, renderização REAL da projeção (mesmas CSS do aluno).
- **Live updates via `BroadcastChannel('aros-slides-edit')`** — editor envia, janela recebe e re-renderiza instantaneamente. Debounce com `requestAnimationFrame` (~60fps).
- Badge verde "AO VIVO" no header da janela.
- Filtro por `slideId` no URL: preview individual mostra só 1 slide.
- `previewSlide(i)` e `previewSlides()` ambos atualizam via canal.

**Parte 2 — Projeção live: 2 bugs críticos**

Bug 1 — **"clique perdido" em Próximo/Anterior** (modo=projecao-live):
- Causa: `state.tickInt` rodava `_atualizaPainelProj(ci)` a cada 500ms, recriando innerHTML do painel inteiro. Se prof clicasse no exato re-render, botão era destruído entre `mousedown` e `mouseup`, click NUNCA disparava.
- Fix: tick agora atualiza só o `textContent` do timer **in-place**. Painel só re-renderiza quando idx muda. A barra fixa do topo (`#proj-fixed-bar`) já tinha esse fix; o painel do caso (`ck-proj-{ci}`) não tinha.

Bug 2 — **double-click acidental durante nav** (round-trip Firestore lento):
- Lock `st._navLocked` em `projecaoNav` previne cliques rápidos. **450ms minimum** (calculado contra `lockedAt`).
- UI: `cursor:wait` + `opacity:.55` + `title="Aguarde…"`.
- Aplicado em painel + barra fixa. Painel respeita via `state._navLocked` no `renderProjPanel`.

Barra de desenho movida pro **topo da projeção**:
- Caneta + paleta de cores + borracha + limpar moveram do canto inferior direito (`position:fixed;bottom;right`) pro **inline dentro do `#proj-timer`**, entre cronômetro e botões de navegação (◀ Ant ▶ Próx PIP Encerrar).
- Visível pra prof E aluno (necessário porque PIP do prof carrega iframe como `role=aluno`).
- CSS: `display:inline-flex` + `border-left:1px solid #333` como separador. HTML `<span>` (não div) embutido em `#proj-timer`.

**Parte 3 — Editor de template do checklist (`renderTplCaso` e adjacências)**

Redesign Apple-like dark glass de TODAS as caixas (caso + pergunta). Detalhe na seção "Editor de Template do Checklist".

Caixa do caso (`.tpl-caso`):
- **Header inteiro clicável** pra abrir/recolher (input/textarea/button mantêm seu comportamento via `closest()` check).
- Removida contagem de perguntas do header.
- Botão "🎬 Slides" → **"🎬 Editar Slides"** com contador entre parênteses.
- **Botão novo "👁 Pré-visualizar"** antes do Editar Slides — abre janela 1280×800 com **todos os slides** do caso (gera padrão via `gerarSlidesPadrao` se ainda não tiver sido customizado). Helper novo: `previewCasoSlides(bloco, ci)`.

Conteúdo:
- Label "📝 Enunciado do caso (opcional)" → **"ENUNCIADO"** (uppercase, JetBrains Mono, letter-spacing, padrão monospace section label).
- Textarea: rows 3 → 6, padding maior.

Caixa da pergunta (`.tpl-perg`):
- Header inteiro clicável também. Visual espelhado do caso, em accent azul.
- Badge "P{n}" pill mono.

Design dark glass (variantes coloridas por bloco):
- `backdrop-filter:blur(14px) saturate(140%)` em cards.
- Borda translúcida via `color-mix` com glow no hover.
- **Faixa lateral colorida** (3px → 5px no hover/open) com `box-shadow` accent.
- Cor do bloco via CSS custom property `--tpl-cor`: azul Criar, laranja Oral.
- Caret animado: `rotate(90deg)` com `cubic-bezier(.4,0,.2,1)`.
- Badge "CASO N" em pill colorido (JetBrains Mono).
- Textarea com `backdrop-blur` + transição no focus.
- Botões `.tpl-btn-action` com hover azul + `translateY(-1px)` (+ variante `.del` vermelha).

Classes CSS novas adicionadas (em sequência depois do `.rt-ed:focus`, área linha 800):
- `.tpl-caso` + `.tpl-caso-head` + `.tpl-caso-num` + `.tpl-caso-input` + `.tpl-caso-body`
- `.tpl-perg` + `.tpl-perg-head` + `.tpl-perg-num` + `.tpl-perg-input` + `.tpl-perg-body`
- `.tpl-caret` + `.tpl-perg-caret`
- `.tpl-section-label`
- `.tpl-textarea`
- `.tpl-btn-action` (+ `.del` variante)

**Commits**: `f2d42ff` + `8e1ea49` (slides + projeção bugs + barra desenho), `5efb26d` (template checklist redesign).

---

### Sessão 2026-05-29 — maratona pré-simulado (painel coord + Recursos + Escala + Checklist)

Sessão longa de polimento e funcionalidades faltando pra rodar o próximo simulado tranquilo. Tudo entregue + deployado. Pacote:

**1. Escala (aluno) — visão pós-deadline.** Box `<details>` "⚠️ Alunos que não confirmaram" no rodapé de sábado/domingo quando `exp=true`. Inclui todos não-confirmados (pending/swap/absent). Auto-ausentes têm `dia=null` mas `originalDia`+`originalRodada` preservados → renderiza chip ao lado do nome. Reusa `alunoRowHTML(s, exp, extraAfterName)` — 3º param novo opcional. Auto-abre em busca. Localização: `renderAlunoDay` ~23282 + `renderNaoConfirmaramBox`. **Detalhe na seção "Visão Aluno".**

**2. Mapa de questões (aluno · Recursos).** Limiar reduzido `>10` → `>=5` em `_renderMapaQuestoes`.

**3. Slider rascunho ↔ publicada no card da prova (Recursos · coord).** Mini-toggle inline em `_recProvaCard` substituiu o chip estático `● PUBLICADA/RASCUNHO`. Click chama `toggleProvaStatus(provaId, el)` — UI otimista + `setDoc merge` + audit `PROVA_STATUS_TOGGLE`. CSS classe `.rec-card-status-toggle` (versão compacta do `.mpv-status-toggle`, não scoped a `#modal-prova`). Publica/despublica sem abrir editor.

**4. Email do parecer · sistema de Recursos.** Template novo `template_recurso_parecer` (service `service_exyoa4r`) — branded MEDREVIEW, Tahoma, light mode. **Detalhe na seção "📧 Template dedicado de parecer".** Helpers novos: `_resolveFonteNome(fonte, fonteCustom)`, `_htmlParaTexto(html)` (strip tags com BR/P/DIV/LI → `\n`). Arquivo de referência: `email-template-recurso.html` na raiz. Deep-link `/recursos?provaRec=X&qRec=Y`.

**5. Painel da Coordenação · sidebar colapsável.** Botão `☰` (`#co-sidebar-toggle-btn`, classe `.co-sidebar-toggle`) ao lado do título "Painel da Coordenação". `toggleCoSidebar()` persiste em `localStorage['aros_co_sidebar_closed']`. `_applyCoSidebarPref()` em `openCoordPanel`+`switchView('coord')`. **Default = fechada na 1ª visita** (quando localStorage null). User info card movido pro topo (wrapper `.co-sidebar-head`); logout fica no foot. Grupos da sidebar também colapsados por default na 1ª visita (`renderSidebar` popula `localStorage['aros-sidebar-collapsed']`). **Detalhe na seção "Visão Coordenação".**

**6. Branding.** Eyebrow do painel coord: "AnestReview" → **"MEDREVIEW"**. Logo trocada `logo-anestreview.png` (URL produção) → `/assets/assets/logo-medreview.png` (caminho absoluto root, funciona local + prod). Arquivo novo em `assets/assets/logo-medreview.png`. **Detalhe nas seções "URLs e endpoints" e "Visão Coordenação".**

**7. Bug fixes pré-simulado (auditoria `aros-reviewer`).**
- **Bug 1 (crítico)** — `_alunoBtnEntrarSala` quebrava se `S.curSim=null`. Fix: assinatura agora `_alunoBtnEntrarSala(s, simId)`; `alunoRowHTML` só renderiza o botão se `S.curSim?.id` existe.
- **Bug 2 (crítico)** — `_projAtivo` não limpo ao trocar de sim no checklist. Fix: `loadCkSim` compara `novoSimId !== _ckSimId` antes de aplicar e roda `await encerrarProjecao(Number(ci))` em todas as entries de `_projAtivo`.
- **Bug 3 (alto)** — `window.open` com feature-string (`width=1280,height=800`) era bloqueado em iOS/Android. Fix: `abrirSalaAluno` removeu 3º arg do `window.open` (mantém named target pra reuso de janela).
- **Bug 4 (alto)** — `saveCkCaso` não bloqueava edição de bloco já finalizado. Fix: `confirm` antes; ao aceitar, reseta `blocoResp.finalizado=false` pra forçar recálculo da nota.
- **Bug 6 (médio)** — chip do dia no box "Não confirmaram" sumia se nome do aluno tivesse `<` ou `>`. Fix: refator `alunoRowHTML` ganhou 3º param `extraAfterName` em vez de regex replace frágil.
- **Bugs 5 e 7 (race/onSnapshot)** NÃO aplicados por risco — mitigação manual (F5 antes de avaliar caso novo se outro prof tiver trabalhando no mesmo sim).

**8. Audit log do checklist (rede de segurança).** 5 pontos críticos do checklist agora gravam snapshot before/after em `auditLog/{auto}`. Helper `_ckSnapshot(v)` = deep clone JSON-safe. Ações: `CK_CASO_SALVO`, `CK_BLOCO_FINALIZADO`, `CK_CASO_RESETADO`, `CK_PERGUNTA_RESETADA`, `CK_ALUNO_RESETADO`. `resetarSimuladoAluno` (a mais destrutiva) lê fresh do Firestore antes do delete pra capturar estado autoritativo (não memória). Audit meta inclui `nomeAluno`, `nomePergunta`, etc. Adicionados em `AUDIT_ACTIONS`: `CK_CASO_SALVO`, `CK_BLOCO_FINALIZADO`, `CK_CASO_RESETADO`, `CK_PERGUNTA_RESETADA`, `CK_ALUNO_RESETADO`, `PROVA_STATUS_TOGGLE`, `AUTO_AUSENTES_POS_PRAZO`.

**9. Regra: itens positivos não marcados → 'errado' ao salvar caso.** `saveCkCaso` agora preenche automaticamente itens POSITIVOS sem valor com `'errado'` antes do save. Itens NEGATIVOS (`it.negativo`) ficam intocados — penalidade exige marcação explícita. `temPendencia` agora sempre `false` (positivos sempre preenchidos). Warning "Há itens não preenchidos" removido. Audit em `CK_CASO_SALVO` registra quantidade via meta `autoFillErrado: N`.

**10. Backup PDF do checklist em branco (template, pro prof aplicador).** Botão "📄 Backup PDF · Criar" e "📄 Backup PDF · Oral" acima de cada bloco card em `renderBlocoCards` (view per-student). `exportCkTemplatePDF(blocoSolo?)` abre nova janela com HTML self-contained pronto pra print. `_renderCkPrintHTML(sim, blocoKey)` renderiza um bloco com cabeçalho (nome/avaliador/data/rodada), casos, perguntas, itens com quadradinhos `☐ Certo` `☐ Parcial` `☐ Errado` (ou `☐ Penalidade` pra negativos), habilidades 1-5, 8 linhas pra feedback, campo nota. **Texto do item vem de `it.descricao`** (não `it.texto` — campo correto descoberto auditando código; fallback `it.texto` mantido por compat). Label do item: `it.label` ou A/B/C automático. CSS print-friendly: `page-break-after:avoid` na meta e no h2 do caso; `page-break-inside:avoid` só na pergunta (granular ideal); caso permite quebra. Footer com timestamp. XSS-safe (caracteres escapados).

**11. Firestore rules — `/provas` afrouxado.** `status in ['rascunho','publicada','ativa','encerrada']` (era só `ativa`/`encerrada` — front migrou pra rascunho/publicada em 2026-05-28, regra estava trancando criação). `tipo is string && tipo.size() > 0` (era enum fixo TSA/TEA/ME1/ME2/ME3) — suporta `tiposCustom` de `recursosConfig.settings`. **Já deployado nessa sessão.** Detalhe na seção "Firestore Rules".

**Commit da sessão**: `d965497` "Painel coord + Recursos + Escala: maratona pré-simulado (sessão 2026-05-29)" + commits subsequentes (`fccd5d9` audit log checklist + `e328e4e` auto-marca positivos como errado). Deploy via push pra `main` (GitHub Pages atualiza `aros.anestreview.com.br`).
...
Home — render instantâneo dos cards + banner de Recursos (2026-05-30):
- **Cards da home instantâneos**: antes `renderHomeCards()` só rodava em `boot()` DEPOIS de `await getDoc(settings)` + `Promise.all([~15 loads])` → cards demoravam. Agora: `loadComunicacaoCards` cacheia `{cards,order}` em `localStorage['aros_comcards']`; `_hydrateComCardsFromCache()` hidrata `S.comunicacao.cards` do cache; e há um **render imediato** logo após a definição de `renderHomeCards` (roda na avaliação do módulo, antes de qualquer Firestore). `renderHomeCards` ganhou **guard de assinatura** (`grid.dataset.sig`) — só re-renderiza se os dados mudaram (evita flicker quando o boot chama de novo com dados iguais ao cache). Stale-while-revalidate: 1ª visita mostra defaults `COM_CATS` na hora; visitas seguintes vêm do cache instantâneas.
- **Banner "Sistema de Recursos AnestReview" na home**: `<button class="home-rec-banner reveal" onclick="goRecursosAluno()">` inserido entre `#home-grid` e `.home-foot`, largura total dos 4 cards. Replica o `.rec-hero` da página /recursos (vidro, grid mesh `::before`, glow roxo `::after` com `recHeroFloat`, cantos `.hrb-corner`, eyebrow + chip Online, brand com gradiente). Classes `.hrb-*` scoped em `#view-home`. Reusa keyframes globais `recHeroFloat`/`recPulse`.

Busca tolerante a acento (2026-05-30): `filterCkStudents` (checklist) e `searchAluno` (escala visão aluno) passaram a usar `_proNormBusca()` nos dois lados da comparação — "mario" acha "Mário" e vice-versa. `_proNormBusca` é função declarada (hoisted), então pode ser chamada antes da definição no arquivo.
