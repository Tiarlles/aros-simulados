...
- **Próximo:** **IA real** de análise por módulo (hoje mockada) usando a transcrição como fonte; e **sincronização automática** Laravel→PO via Cloud Function quando o dev der um **token de API estável** (não usar login/senha; o token Sanctum capturado é pessoal/temporário). ⚠️ Tokens colados no chat e orientados a **regenerar**: Sanctum do Laravel (`3044609|…`) e **Vimeo PAT** (`6eed…342f`, conta OFT-Review) + Client secret do app Vimeo.
